			  ...
			  struct per_cpu_info *pci)
{
	...
	switch (act) {
		...
		case __BLK_TA_INSERT:
			...
			snprintf(buf, sizeof(buf), "%llu %u", (unsigned long long)t->sector, ((t)->bytes)>>12);
			write(mq, buf, strlen(buf));
			break;
...
int main(int argc, char *argv[])
{
	...
	if (pipeline) 
	{
		char* pipeName = "/tmp/blkpipe";
		mkfifo(pipeName, 0666);
		mq = open(pipeName, O_WRONLY);
	...
	if (pipeline) 
	{
		const char *closeString = "end";
		write(mq, closeString, strlen(closeString));
		close(mq);
	}
